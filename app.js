...
async function loadMyNodes() {
    ...
    try {
        ...
        container.innerHTML = data.map(node => `<div class="node-card">${node.logo_url ? `<img src="${escapeHTML(node.logo_url)}" alt="" aria-hidden="true" style="display:block;width:100%;max-height:220px;object-fit:contain;margin:0 auto;">` : ""}<div class="node-card-content"><h3>${escapeHTML(node.name)}</h3><p>${escapeHTML(node.city || "")}, ${escapeHTML(node.country || "")}</p>${node.upland_location ? `<p>Neighborhood: <strong>${escapeHTML(node.upland_location)}</strong></p>` : ""}${node.upland_node_url ? `<p><a href="${safeURL(node.upland_node_url)}" target="_blank" rel="noopener noreferrer">Open Upland Node Link</a></p>` : ""}<p>Status: <strong>${escapeHTML(node.status || "pending")}</strong></p>${node.status === "approved" ? `<p><a href="#node/${encodeURIComponent(node.id)}">View public profile</a></p>` : ""}</div></div>`).join("");
