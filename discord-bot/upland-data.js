...
function findTextChannel(guild, name) {
  return guild.channels.cache.find(channel =>
    channel.isTextBased?.() && channel.name === name
  );
}
...
async function messageExists(channel, marker) {
  ...
  return Boolean(recent?.some(message => message.author?.bot && message.embeds?.some(embed =>
...
async function publishDailyRanking(guild, rows) {
  ...
  const lines = ranking.map((item, index) =>
    `**${index + 1}. ${item.username}** — ${upx(item.reward)} · ${item.treasures} treasure${item.treasures === 1 ? '' : 's'}`
  );
...
async function publishGuide(guild) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription([
      ...
      '',
      '**Important**',
      'Only information available through the authorized Upland data sources will be published. Private account data requires the player to authorize the Node Hub application.',
    ].join('\n'))
    .setFooter({ text: marker })
    .setTimestamp();
...
async function playerStats(guild, ign) {
  const channel = findTextChannel(guild, CHANNELS.playerStats);
  ...
  if (!playerRows.length) {
    return new EmbedBuilder().setTitle('PLAYER STATS').setDescription(`No public Treasure Hunt data found for **${ign}**.`);
  }

  const total = playerRows.reduce((sum, row) => sum + Number(row.reward || 0), 0);
  const types = new Map();
  for (const row of playerRows) types.set(row.treasureType || 'unknown', (types.get(row.treasureType || 'unknown') || 0) + 1);
...
async function registerDataCommands() {
  if (!CLIENT_ID || !process.env.DISCORD_TOKEN) return;
  const rest = new REST({ version: '10' }).setToken(process.env.DISCORD_TOKEN);
  const command = new SlashCommandBuilder()
    .setName('player-stats')
    .setDescription('Show public Upland Treasure Hunt statistics for a player.')
    .addStringOption(option => option
      .setName('igname')
      .setDescription('Upland IG Name, exactly as shown in the game.')
      .setRequired(true))
    .toJSON();

  if (GUILD_ID) {
    const existing = await rest.get(Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID));
    const filtered = existing.filter(item => item.name !== 'player-stats');
...
function setupUplandData(client) {
  client.once('ready', async () => {
    try {
      ...
      for (const guild of client.guilds.cache.values()) {
        ...
        const rows = await fetchRecentTreasures(10).catch(error => {
          console.error('UPLAND_DATA_FETCH_ERROR:', error.message);
          return [];
        });
  ...
  client.on('interactionCreate', async interaction => {
    ...
    try {
      const embed = await playerStats(interaction.guild, ign);
