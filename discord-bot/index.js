...
if (!discordEnabled) {
  console.warn('Discord bot credentials are not configured. Starting webhook server only.');
}

const client = discordEnabled ? new Client({ intents: [GatewayIntentBits.Guilds] }) : null;
...
const TEAM_STRUCTURE = [['team-chat', 'text'], ['tasks', 'text'], ['development', 'text'], ['internal-bugs', 'text'], ['node-status', 'text'], ['event-log', 'text']];
...
async function getOrCreateRole(guild, definition) { let role = guild.roles.cache.find(r => r.name === definition.name); if (!role) role = await guild.roles.create({ name: definition.name, color: definition.color, reason: 'Node Hub initial Discord setup' }); return role; }
...
async function getOrCreateTextChannel(guild, name, parent, privateChannel = false, teamRoleIds = []) { const channelName = slug(name); let channel = guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === channelName); if (!channel) channel = await guild.channels.create({ name: channelName, type: ChannelType.GuildText, parent: parent.id, reason: 'Node Hub initial Discord setup' }); else if (channel.parentId !== parent.id) await channel.setParent(parent.id, { lockPermissions: false }); if (privateChannel) { await channel.permissionOverwrites.edit(guild.roles.everyone, { ViewChannel: false }); for (const roleId of teamRoleIds) await channel.permissionOverwrites.edit(roleId, { ViewChannel: true, SendMessages: true, ReadMessageHistory: true }); } return channel; }
async function setupServer(guild) { const roles = {}; for (const definition of ROLE_DEFINITIONS) roles[definition.name] = await getOrCreateRole(guild, definition); const teamRoleIds = ROLE_DEFINITIONS.filter(r => TEAM_ROLES.has(r.name)).map(r => roles[r.name].id); for (const section of PUBLIC_STRUCTURE) { const category = await getOrCreateCategory(guild, section.name); for (const [name] of section.channels) await getOrCreateTextChannel(guild, name, category); } const teamCategory = await getOrCreateCategory(guild, '🟧 TEAM', true, teamRoleIds); for (const [name] of TEAM_STRUCTURE) await getOrCreateTextChannel(guild, name, teamCategory, true, teamRoleIds); const logChannel = guild.channels.cache.find(c => c.type === ChannelType.GuildText && c.name === 'event-log'); if (logChannel) await logChannel.send({ content: 'Node Hub event logging is ready. System events will appear here automatically.' }); return { roles, teamCategory }; }
...
if (discordEnabled) {
  ...
  client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'setup-server') { if (!interaction.memberPermissions?.has(PermissionFlagsBits.Administrator)) return interaction.reply({ content: 'Administrator permission is required.', ephemeral: true }); await interaction.deferReply({ ephemeral: true }); try { await setupServer(interaction.guild); await interaction.editReply('Node Hub server structure created successfully.'); } catch (error) { console.error(error); await interaction.editReply(`Setup failed: ${error.message}`); } }
