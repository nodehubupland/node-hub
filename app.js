...
// Public V1 initialization. Language is handled by the lightweight selector in index.html.
...
async function getCurrentRole(){
    ...
    try{const{data,error}=await db.from("profiles").select("role").eq("id",currentUser.id).maybeSingle();if(error)return "user";return data?.role||"user";}catch{return "user";}
}
...
function renderWorldMap(list){
    const stats=document.getElementById("node-map-stats"),canvas=document.getElementById("node-map-canvas"),listEl=document.getElementById("node-map-list");
    if(!canvas||!listEl)return;
    if(stats)stats.textContent=`${list.length} verified ${list.length===1?"Node":"Nodes"}`;
    canvas.innerHTML="";
    if(!list.length){canvas.innerHTML=`<div class="node-map-empty">No verified Nodes yet.</div>`;listEl.innerHTML="";return;}
    const dots=list.map((node,index)=>{const left=12+((index*43)%76),top=18+((index*59)%62);return `<a href="#node/${encodeURIComponent(node.id)}" title="${escapeHTML(node.name||"Node")}" style="position:absolute;left:${left}%;top:${top}%;width:11px;height:11px;border-radius:50%;background:var(--accent-light);box-shadow:0 0 0 5px rgba(255,132,0,.12);z-index:2;"></a>`;}).join("");
    canvas.innerHTML=`<div style="position:absolute;inset:18% 10%;border:1px solid rgba(255,255,255,.08);border-radius:48%;transform:rotate(-8deg);"></div><div style="position:absolute;inset:28% 18%;border:1px solid rgba(255,255,255,.06);border-radius:42%;transform:rotate(12deg);"></div>${dots}<span style="position:absolute;left:16px;bottom:14px;font-size:10px;opacity:.5;letter-spacing:.12em;text-transform:uppercase;">Node locations</span>`;
    listEl.innerHTML=list.map(node=>`<a href="#node/${encodeURIComponent(node.id)}" class="node-map-list-item"><strong>${escapeHTML(node.name||"Unnamed Node")}</strong><small>${escapeHTML([node.city,node.country].filter(Boolean).join(", ")||"Location not provided")}</small></a>`).join("");
}

function setupAuthForms(){
    const signup=document.getElementById("signup-form"),login=document.getElementById("login-form");
...
async function signUp(){const username=document.getElementById("signup-username")?.value.trim()||"";const email=document.getElementById("signup-email")?.value.trim().toLowerCase()||"";const password=document.getElementById("signup-password")?.value||"";if(!email||!password){alert("Please enter your email and password.");return;}if(password.length<8){alert("Password must contain at least 8 characters.");return;}try{const{data,error}=await db.auth.signUp({email,password,options:{emailRedirectTo:"https://nodehubupland.github.io/node-hub/",data:{username}}});if(error){alert(error.message);return;}if(data.user&&data.session){currentUser=data.user;await createProfile(username);updateAuthUI();window.location.hash="dashboard";}else if(data.user)alert("Account created. Please check your email to confirm your account.");}catch(error){console.error(error);alert("Unable to create your account.");}}
