...
function handleRoute() {
    ...
    if (hash === "register") {
        ...
        showDashboard();
        setTimeout(() => {
            document.getElementById("dashboard-node-form")?.scrollIntoView({ behavior: "smooth" });
        }, 100);
        return;
    }
...
async function signUp() {
    ...
    try {
        const { data, error } = await db.auth.signUp({
            ...
            password,
            options: {
                emailRedirectTo: "https://nodehubupland.github.io/node-hub/",
                data: { username }
            }
        });
...
async function loadNodes() {
    try {
        ...
        console.error("Unexpected Node loading error:", error);
        showEmptyDirectory();
    }
}
...
function renderNodes(list) {
    ...
    if (nodeCount) {
        nodeCount.textContent = `${list.length} ${list.length === 1 ? "Node" : "Nodes"}`;
    }
...
function createNodeCard(node) {
    ...
    const imageHTML = logo
        ? `<div class="node-card-image"><img src="${escapeHTML(logo)}" alt="${escapeHTML(node.name || "Node")}" loading="lazy"></div>`
...
function showEmptyDirectory() {
    ...
    nodeGrid.innerHTML = `
        <div class="empty-directory">
            ...
            <h3>No Nodes listed yet</h3>
            <p>Be one of the first Node administrators to register your Node.</p>
            <a href="#submit" class="button button-primary">Submit a Node</a>
        </div>
    `;
...
function filterNodes() {
    ...
    const filtered = nodes.filter(node => {
        const text = [node.name, node.city, node.country, node.upland_location, node.description]
    ...
    section.innerHTML = `
        <div class="container">
            <div class="auth-card">
                ...
                <h2>Loading Node...</h2>
                <p>Loading verified Node information.</p>
            </div>
        </div>
    `;
        ...
        section.innerHTML = `
            <div class="container">
                <div class="auth-card" style="text-align:center;">
                    ${logo ? `<img src="${escapeHTML(logo)}" alt="${escapeHTML(node.name)}" style="max-width:280px;max-height:280px;object-fit:contain;margin:0 auto 24px;">` : ""}
...
function removeNodeProfile() {
    document.getElementById("nodehub-node-profile")?.remove();
}
...
function createDashboard() {
    ...
    section.innerHTML = `
        <div class="container">
            ...
            <div class="auth-card">
                <span class="eyebrow">YOUR NODES</span>
                <h3>My Nodes</h3>
                <div id="my-nodes">Loading your Nodes...</div>
            </div>
...
async function loadMyNodes() {
    ...
    if (!container) return;
    if (!currentUser) {
        container.textContent = "Please sign in.";
        return;
    }
    ...
    try {
        ...
        if (error) {
            console.error("My Nodes error:", error);
            container.innerHTML = `<p>Unable to load your Nodes.</p>`;
            return;
        }

        if (!data?.length) {
            container.innerHTML = `<p>No Nodes registered yet.</p><p>Use "Register My Node" to submit your first Node.</p>`;
            return;
        }

        container.innerHTML = data.map(node => `
            <div class="node-card">
                ${node.logo_url ? `<img src="${escapeHTML(node.logo_url)}" alt="${escapeHTML(node.name)}" style="width:100%;max-height:220px;object-fit:contain;">` : ""}
                <div class="node-card-content">
        ...
        console.error("Unexpected My Nodes error:", error);
    }
}
...
function setupNodeRegistration() {
    ...
    document.getElementById("cancel-node-registration")?.addEventListener("click", () => {
        window.location.hash = "home";
    });
}
...
async function submitNode() {
    ...
    if (uplandNodeURL && !isValidURL(uplandNodeURL)) {
...
function removeDashboard() {
    document.getElementById("nodehub-dashboard")?.remove();
}
...
function setupLanguage() {
    ...
    selector.addEventListener("change", () => {
        ...
        if (language === "pt-BR") {
            alert("Português (BR) translation will be activated in the next development stage.");
        }
    });
