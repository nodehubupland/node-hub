...
async function register() { if (!CLIENT_ID || !DISCORD_TOKEN) return; const rest = new REST({ version: '10' }).setToken(DISCORD_TOKEN); await rest.put(Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID), { body: buildCommands() }); }
function connectionButton() { return new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel('Connect Upland Account').setStyle(ButtonStyle.Link).setURL('https://nodehubupland.github.io/node-hub/#dashboard')); }
async function fetchTreasure(ign) {
  const url = new URL(TREASURE_URL); url.searchParams.set('ign', ign);
  ...
  try {
    ...
    if (!response.ok) { const error = new Error(body?.error || `UPLAND_TREASURE_HTTP_${response.status}`); error.status = response.status; throw error; }
    return body || {};
  } catch (error) { if (error?.name === 'AbortError') { const timeout = new Error('TREASURE_TIMEOUT'); timeout.code = 'TREASURE_TIMEOUT'; throw timeout; } throw error; }
...
function historyEmbed(ign, data) {
  ...
  if (!rows.length) { embed.addFields({ name: 'Result', value: 'No public Treasure Hunt history found for this player.' }); if (data.partial) embed.setFooter({ text: `Search limited to the most recent ${data.pages_limit ? data.pages_limit * 10 : 300} records.` }); return embed; }
  const lines = rows.slice(0, 10).map((r, i) => { const date = r.lockedAt || r.spawnAt || 'Date unavailable'; const address = r.fullAddress || 'Address unavailable'; const reward = r.reward != null ? `${r.reward} UPX` : 'Reward unavailable'; const type = r.treasureType || 'treasure'; return `**${i + 1}.** ${date}\n${type} • ${reward}\n${address}`; });
...
async function setup(client) {
  ...
  client.on('interactionCreate', async interaction => {
    ...
    try { const data = await fetchTreasure(ign); if (data.connected === false || data.error === 'PLAYER_NOT_CONNECTED') return interaction.editReply(errorMessage(Object.assign(new Error('PLAYER_NOT_CONNECTED'), { status: 404 }), ign)); return interaction.editReply({ embeds: [historyEmbed(ign, data)] }); }
    catch (error) { console.error(`TREASURE_COMMAND_ERROR:${ign}:`, error); return interaction.editReply(errorMessage(error, ign)); }
  });
}
