...
async function initializeAuth() {
    try {
        ...
        db.auth.onAuthStateChange((event, session) => {
            ...
            updateAuthUI();
            if (event === "SIGNED_IN") {
                setTimeout(() => { loadNodes(); updateDashboard(); handleRoute(); }, 0);
            }
            if (event === "SIGNED_OUT") {
                removeDashboard();
                removeNodeProfile();
...
function handleRoute() {
    ...
    if (hash.startsWith("node/")) {
        setDashboardMode(false);
        const nodeId = decodeURIComponent(hash.substring(5));
        removeDashboard();
        showNodeProfile(nodeId);
        return;
    }
    removeNodeProfile();
    if (hash === "dashboard") {
...
async function createProfile(username) {
    ...
    try {
        const { error } = await db.from("profiles").upsert({ id: currentUser.id, username: username || currentUser.email?.split("@")[0] || "User", email: currentUser.email }, { onConflict: "id" });
        if (error) console.warn("Profile creation warning:", error);
...
function setupAuthForms() {
    ...
    if (loginForm) loginForm.addEventListener("submit", event => { event.preventDefault(); signIn(); });
}
...
async function loadNodes() {
    try {
        ...
        if (error) { console.error("Error loading Nodes:", error); showEmptyDirectory(); return; }
        nodes = data || [];
        renderNodes(nodes);
...
function createNodeCard(node) {
    ...
    const imageHTML = logo ? `<div class="node-card-image"><img src="${escapeHTML(logo)}" alt="${escapeHTML(node.name || "Node")}" loading="lazy"></div>` : `<div class="node-card-image node-card-placeholder">●</div>`;
    article.innerHTML = `
        ${imageHTML}
        <div class="node-card-content">
            ...
            <div class="node-links">
                ...
                <a href="#node/${encodeURIComponent(node.id)}">View Node Profile</a>
                ${node.upland_node_url ? `<a href="${safeURL(node.upland_node_url)}" target="_blank" rel="noopener noreferrer">Open in Upland</a>` : ""}
            </div>
            <div class="node-card-footer"><span class="verified-badge">Verified Node</span></div>
        </div>`;
    return article;
}
...
function filterNodes() {
    ...
    const continent = continentFilter ? continentFilter.value : "";
    ...
    section.innerHTML = `
        <div class="container">
            <div class="section-heading">
                <span class="eyebrow">NODE HUB ACCOUNT</span>
                <h2>Dashboard</h2>
                <p>Manage your Node Hub account and Node submissions.</p>
                <button type="button" id="dashboard-logout-button" data-action="logout" class="button button-secondary">Sign out</button>
            </div>
            <div class="auth-card"><span class="eyebrow">YOUR NODES</span><h3>My Nodes</h3><div id="my-nodes">Loading your Nodes...</div></div>
            <div id="dashboard-node-form" class="auth-card">
                <span class="eyebrow">NODE REGISTRATION</span>
                <h3>Register My Node</h3>
                <p>Node registration will be submitted for review by the Node Hub team.</p>
                <form id="node-registration-form">
                    <label for="node-name">Node Name</label>
                    <input type="text" id="node-name" required placeholder="Node name">
                    <label for="node-description">Description</label>
                    <textarea id="node-description" rows="4" placeholder="Tell us about your Node"></textarea>
                    <label for="node-city">City</label>
                    <input type="text" id="node-city" required placeholder="City">
                    ...
                    <small>Paste the direct link to the Node property in Upland.</small>
                    <label for="node-continent">Continent</label>
                    <select id="node-continent" required>
                        <option value="">Select continent</option><option value="North America">North America</option><option value="South America">South America</option><option value="Europe">Europe</option><option value="Asia">Asia</option><option value="Africa">Africa</option><option value="Oceania">Oceania</option>
                    </select>
                    <label for="node-logo">Node Logo</label>
                    <input type="file" id="node-logo" accept="image/png,image/jpeg,image/webp">
                    <small>PNG, JPG or WEBP. Maximum 5 MB.</small>
                    <div id="node-image-preview"></div>
                    <label for="node-discord">Discord</label>
                    <input type="url" id="node-discord" placeholder="https://discord.gg/...">
                    <label for="node-twitter">X / Twitter</label>
                    <input type="url" id="node-twitter" placeholder="https://x.com/...">
                    <label for="node-telegram">Telegram</label>
                    <input type="url" id="node-telegram" placeholder="https://t.me/...">
...
async function updateDashboard() {
    ...
    try {
        const { data } = await db.from("profiles").select("username").eq("id", currentUser.id).maybeSingle();
        if (data?.username) username = data.username;
    } catch (error) { console.warn("Could not load profile:", error); }
    if (welcome) welcome.textContent = `Welcome, ${username || "Node Administrator"}`;
    updateAuthUI();
    await loadMyNodes();
}
...
async function loadMyNodes() {
    ...
    try {
        ...
        if (error) { console.error("My Nodes error:", error); container.innerHTML = `<p>Unable to load your Nodes.</p>`; return; }
        if (!data?.length) { container.innerHTML = `<p>No Nodes registered yet.</p><p>Use "Register My Node" to submit your first Node.</p>`; return; }
        container.innerHTML = data.map(node => `<div class="node-card">${node.logo_url ? `<img src="${escapeHTML(node.logo_url)}" alt="${escapeHTML(node.name)}" style="width:100%;max-height:220px;object-fit:contain;">` : ""}<div class="node-card-content"><h3>${escapeHTML(node.name)}</h3><p>${escapeHTML(node.city || "")}, ${escapeHTML(node.country || "")}</p>${node.upland_location ? `<p>Neighborhood: <strong>${escapeHTML(node.upland_location)}</strong></p>` : ""}${node.upland_node_url ? `<p><a href="${safeURL(node.upland_node_url)}" target="_blank" rel="noopener noreferrer">Open Upland Node Link</a></p>` : ""}<p>Status: <strong>${escapeHTML(node.status || "pending")}</strong></p>${node.status === "approved" ? `<p><a href="#node/${encodeURIComponent(node.id)}">View public profile</a></p>` : ""}</div></div>`).join("");
...
function setupNodeRegistration() {
    ...
    const imageInput = document.getElementById("node-logo");
    if (imageInput) imageInput.addEventListener("change", previewNodeImage);
    form.addEventListener("submit", async event => { event.preventDefault(); await submitNode(); });
...
function previewNodeImage(event) {
    ...
    if (file.size > 5 * 1024 * 1024) { alert("The image must be smaller than 5 MB."); event.target.value = ""; return; }
...
async function submitNode() {
    ...
    const uplandNodeURL = get("node-upland-url");
    ...
    if (uplandNodeURL && !isValidURL(uplandNodeURL)) { alert("Please enter a valid Upland Node Link."); return; }
    const imageFile = document.getElementById("node-logo")?.files?.[0] || null;
    const submitButton = document.querySelector("#node-registration-form button[type='submit']");
    ...
    try {
        ...
        alert("Unable to submit the Node: " + error.message);
    } finally {
...
function clearNodeForm() {
    document.getElementById("node-registration-form")?.reset();
    const preview = document.getElementById("node-image-preview");
...
function setupLanguage() {
    ...
    selector.addEventListener("change", () => {
        ...
        localStorage.setItem("nodehub-language", language);
        if (language === "pt-BR") alert("Português (BR) translation will be activated in the next development stage.");
    });
    const savedLanguage = localStorage.getItem("nodehub-language");
    if (savedLanguage) selector.value = savedLanguage;
}
...
function safeURL(url) {
    return isValidURL(url) ? escapeHTML(new URL(url).href) : "#";
}
