...
const DAILY_RANKING_FUNCTION = String(process.env.UPLAND_DAILY_RANKING_URL || 'https://ynqtzyzxspoxssjrjeve.supabase.co/functions/v1/upland-daily-ranking').trim();
...
async function requestJson(baseUrl, params = {}, timeout = 30000) {
  ...
  try {
    ...
    if (error.name === 'AbortError') throw new Error('UPLAND_FUNCTION_TIMEOUT');
    throw error;
  } finally { clearTimeout(timer); }
}

async function fetchTreasure(ign, city = '') { return requestJson(TREASURE_FUNCTION, { ign, city }, 20000); }
async function fetchAnalytics(period = '24h') { return requestJson(ANALYTICS_FUNCTION, { period }, 60000); }
async function fetchDailyRanking() { return requestJson(DAILY_RANKING_FUNCTION, {}, 60000); }
...
function dailyRankingEmbed(body) {
  ...
  const ranking = Array.isArray(body.ranking) ? body.ranking.slice(0, 30) : [];
  ...
  return new EmbedBuilder().setTitle(`DAILY TREASURE RANKING · ${date}`).setDescription(description.slice(0, 4096)).setFooter({ text: `NODEHUB_DAILY_RANKING:${date}` }).setTimestamp();
}

async function updateDailyRanking(client) {
  if (!DAILY_RANKING_CHANNEL_ID) { console.warn('DAILY_RANKING_CHANNEL_ID is not configured; hourly ranking is disabled.'); return; }
  try {
    ...
    const body = await fetchDailyRanking();
    ...
    const messageEmbed = dailyRankingEmbed(body);
    if (message) {
      await message.edit({ content: '', embeds: [messageEmbed] });
      console.log('DAILY_RANKING_UPDATED', message.id);
    } else {
      const sent = await channel.send({ embeds: [messageEmbed] });
      console.log('DAILY_RANKING_CREATED', sent.id);
      console.log(`Set DAILY_RANKING_MESSAGE_ID=${sent.id} in Render to keep editing this same message.`);
    }
  } catch (error) { console.error('DAILY_RANKING_UPDATE_ERROR:', error.message); }
}

async function startDailyRanking(client) {
  if (!DAILY_RANKING_CHANNEL_ID) return;
  await updateDailyRanking(client);
  setInterval(() => updateDailyRanking(client), 60 * 60 * 1000);
}
...
async function register() {
  ...
  await rest.put(Routes.applicationGuildCommands(CLIENT_ID, GUILD_ID), { body: [
    new SlashCommandBuilder().setName('treasure').setDescription('Show Upland Treasure Hunt history for a player').addStringOption(o => o.setName('ign').setDescription('Upland IGN').setRequired(true)).addStringOption(o => o.setName('city').setDescription('Optional city name').setRequired(false)).toJSON(),
    new SlashCommandBuilder().setName('treasure-global').setDescription('Show global Upland Treasure Hunt analytics').addStringOption(o => o.setName('period').setDescription('Analytics period').setRequired(false).addChoices({ name: '24 hours', value: '24h' }, { name: '48 hours', value: '48h' }, { name: '7 days', value: '7d' })).toJSON(),
  ] });
}
...
async function setupUplandData(client) {
  ...
  client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'treasure') {
      ...
      } catch (error) { console.error(`TREASURE_COMMAND_ERROR:${ign}:`, error.message); await interaction.editReply({ content: `Unable to retrieve Upland Treasure Hunt data: **${error.message}**` }); }
      return;
    }
    if (interaction.commandName === 'treasure-global') {
      const period = interaction.options.getString('period', false) || '24h';
      await interaction.deferReply();
      try { const body = await fetchAnalytics(period); await interaction.editReply({ embeds: [analyticsEmbed(body)] }); }
      catch (error) { console.error(`TREASURE_GLOBAL_COMMAND_ERROR:${period}:`, error.message); await interaction.editReply({ content: `Unable to retrieve global Treasure Hunt analytics: **${error.message}**` }); }
    }
  });
}
