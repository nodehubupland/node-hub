...
function reward(value) { return `${Number(value || 0).toLocaleString('en-US')} UPX`; }
function dateOf(row) { return new Date(row.lockedAt || row.spawnAt || 0); }
...
function resultEmbed(rows, ign) {
  ...
  return new EmbedBuilder().setTitle('TREASURE HUNT RESULT').setDescription([`**Player:** ${sorted[0]?.userName || ign}`, `**Treasures found:** ${sorted.length}`, `**Total rewards:** ${reward(total)}`, '', '**History**', ...lines].join('\n')).setFooter({ text: 'Node Hub · Upland' }).setTimestamp();
}

function connectPrompt(ign) {
  return { content: `**${ign}** is not connected to Node Hub yet. Connect the Upland account on Node Hub, authorize the application, then run **/treasure ${ign}** again.`, components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel('Connect Upland Account').setStyle(ButtonStyle.Link).setURL(NODE_HUB_URL))] };
}
...
function setupUplandData(client) {
  client.once('ready', async () => {
    console.log(`UPLAND_V4_READY: appId=${Boolean(APP_ID)} accessToken=${Boolean(APP_ACCESS_TOKEN)} treasureService=${TREASURE_SERVICE}`);
  ...
  client.on('interactionCreate', async interaction => {
    ...
    try {
      ...
      await interaction.editReply({ embeds: [new EmbedBuilder().setTitle('PLAYER STATS').setDescription([`**Player:** ${rows[0].userName || ign}`, `**Treasures found:** ${rows.length}`, `**Total rewards:** ${reward(total)}`, `**Average reward:** ${reward(total / rows.length)}`, `**Best reward:** ${reward(best)}`].join('\n')).setTimestamp()] });
    } catch (error) {
