...
const ROLE_DEFINITIONS = [
  ['Founder', 0xF5A623, 'admin'],
  ['Administrator', 0xE74C3C, 'admin'],
  ['Bot', 0x5865F2, 'admin'],
  ['Moderator', 0x2ECC71, 'moderator'],
  ['Developer', 0x3498DB, 'moderator'],
  ['Partner', 0x1ABC9C, 'basic'],
  ['Creator', 0x9B59B6, 'basic'],
  ['Legend', 0xF1C40F, 'basic'],
  ['Elite', 0xE67E22, 'basic'],
  ['Contrib', 0x9B59B6, 'basic'],
  ['Active', 0x2ECC71, 'basic'],
  ['Member', 0x7F8C8D, 'basic'],
  ['Uplander', 0x95A5A6, 'basic'],
  ['Verified', 0xF1C40F, 'basic'],
  ['Upland Listings', 0x3498DB, 'basic'],
  ['Treasure Hunt', 0xF1C40F, 'basic'],
  ['Upland News', 0xE74C3C, 'basic'],
  ['Node Hub Content', 0x9B59B6, 'basic'],
  ['Community', 0x1ABC9C, 'basic'],
];
...
async function positionRoles(guild, roles) {
  ...
  const developer = roles.get('Developer');
  if (!botRole) return;
  const target = Math.max(moderator?.position || 1, developer?.position || 1) + 1;
  if (botRole.editable && botRole.position <= target) {
    await botRole.setPosition(target, 'Place Node Hub Bot above moderation roles').catch(() => {});
  }
  if (moderator?.editable && developer?.editable && moderator.position < developer.position) {
    await moderator.setPosition(developer.position, 'Keep moderation hierarchy consistent').catch(() => {});
  }
}
...
async function sync(guild) {
  ...
  if (me && roles.get('Moderator') && me.roles.highest.position <= roles.get('Moderator').position) {
    console.warn('NODE_HUB_ROLE_WARNING: Bot role is not above Moderator. Discord may require manual role ordering.');
  }
